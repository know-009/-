...
def rank_us_themes(
    ...
    for theme in themes:
        ...
        # 절대 거래대금은 로그로 눌러 초대형주 독식을 줄이고,
        # 평소 대비 거래대금 증가율·상승 확산도·가중 등락률을 함께 평가한다.
        score = (
            math.log10(total_value / 1_000_000 + 1) * 8
            + max(-15, min(40, (value_ratio - 1) * 25))
            + breadth * 20
            + max(-20, min(20, weighted_change * 1.5))
        )
        ranked.append(
            ThemeResult(
                name=theme["name"],
...
def fetch_listing_dates() -> dict[str, date]:
    ...
    date_col = next(
        (c for c in ("ListingDate", "Listing Date", "상장일") if c in listing.columns),
        None,
    )
    if not code_col or not date_col:
        raise RuntimeError(
            "KRX 상장일 데이터 열을 찾지 못했습니다. "
            f"현재 열: {', '.join(map(str, listing.columns))}"
        )

    result: dict[str, date] = {}
    for code, raw_date in zip(listing[code_col], listing[date_col]):
        parsed = pd.to_datetime(raw_date, errors="coerce")
        if pd.notna(parsed):
            result[str(code).zfill(6)] = parsed.date()
    if not result:
        raise RuntimeError("KRX 상장일 데이터를 한 건도 읽지 못했습니다.")
    return result
...
def get_new_listing_day(
    ...
    """상장일을 1일차로 하여 첫 3거래일에만 1~3을 반환한다."""
    listing_date = listing_dates.get(ticker)
    if listing_date is None or target_date < listing_date:
        return None

    # 첫 3거래일이 14달력일을 넘는 극단적인 장기 휴장은 현실적으로 없다.
    if (target_date - listing_date).days > 14:
        return None
...
def fetch_kr_market_top10(
    target_date: date, listing_dates: dict[str, date]
) -> list[dict[str, Any]]:
        ...
        results.append(
            {
                ...
                "new_listing_day": get_new_listing_day(
                    ticker, target_date, listing_dates
                ),
            }
        )
        if len(results) == 10:
...
def build_messages(
    ...
    header = [
        ...
        "━━━━━━━━━━━━━━━━━━━━━━━━━━━━",
        f"1. 미국 주도 테마 유니버스 TOP 3 (미국 기준 {us_date_text})",
        "거래대금 규모 + 20일 평균 대비 증가율 + 상승 확산도 + 가중 등락률",
    ]
    for index, theme in enumerate(themes, 1):
        ...
        header.extend(
            [
                f"{index}) {theme.name}",
                f"- 미 주요 종목: {leader_text}",
                f"- 테마 거래대금 강도: {theme.value_ratio:.2f}배 / 가중 등락률 {_signed(theme.weighted_change_pct)}",
                f"- 국내 연계주: {kr_text}",
            ]
        )
...
def main() -> None:
    ...
    kr_top10 = fetch_kr_market_top10(target_date, listing_dates)
