...
def _us_stock_label(ticker: str) -> str:
    name = US_STOCK_NAMES_KO.get(ticker, ticker)
    if name == ticker:
        return html.escape(ticker)
    return html.escape(f"{name}({ticker})")


def _toss_kr_stock_link(name: str, code: str) -> str:
...
def build_messages(
    ...
    header = [
        f"📅 <b>{target_date.isoformat()} 모닝 마켓 브리핑</b>",
        "⏰ 07:50",
        "━━━━━━━━━━━━━━━━━━━━",
        "",
        "🇺🇸 <b>미국 마지막 거래일 주도 테마 TOP 3</b>",
        f"🗓 미국 기준 <code>{us_date_text}</code>",
        "<i>기준: 상승률 · 거래대금 · 20일 평균 대비 거래대금</i>",
    ]
    medals = ("🥇", "🥈", "🥉")
    for index, theme in enumerate(themes, 1):
        ...
        leader_lines = [
            (
                f"{_trend_icon(item.change_pct)} <b>{_us_stock_label(item.ticker)}</b>  "
                f"<b>{_signed(item.change_pct)}</b>  | 거래대금 {item.value_ratio:.1f}배"
            )
            for item in leaders
        ]
        kr_text = " · ".join(
            _toss_kr_stock_link(item["name"], item["code"])
            for item in theme.kr_stocks
        )
        header.extend(
            [
                "",
                f"{medals[index - 1]} <b>{html.escape(theme.name)}</b>",
                "",
                "<b>미국 주도주</b>",
                *leader_lines,
                "",
                "<b>테마 흐름</b>",
                f"• 거래대금 강도  <b>{theme.value_ratio:.2f}배</b>",
                (
                    f"• 가중 등락률  {_trend_icon(theme.weighted_change_pct)} "
                    f"<b>{_signed(theme.weighted_change_pct)}</b>"
                ),
                f"• 상승 확산  <b>{theme.breadth * 100:.0f}%</b>",
                "",
                "<b>국내 연결</b>",
                kr_text,
                "━━━━━━━━━━━━━━━━━━━━",
    ...
    domestic = [
        "🇰🇷 <b>국내 거래대금 TOP 10</b>",
        "<i>KOSPI 시총 1~3위·ETF·ETN·스팩·레버리지·인버스·우선주 제외</i>",
        "",
    ]
    duplicates: list[str] = []
    for index, item in enumerate(kr_top10, 1):
        ...
        duplicate_tag = (
            f"  ⭐ <b>미국 연계: {html.escape(theme_name)}</b>" if theme_name else ""
        )
        if theme_name:
            duplicates.append(
                f"{_toss_kr_stock_link(item['name'], item['ticker'])} · "
                f"{html.escape(theme_name)}"
            )
        stock_link = _toss_kr_stock_link(item["name"], item["ticker"])
        domestic.append(
            f"{index:02d}. {new_tag}{stock_link} "
            f"<code>[{html.escape(item['market'])}]</code> "
            f"{_trend_icon(item['change'])} <b>{_signed(item['change'])}</b> "
            f"| {item['amount_eok']:,}억{duplicate_tag}"
        )
    ...
    cross.extend(
        [
            "",
            "<i>※ 신규 태그는 상장 당일을 1일차로 계산해 첫 3거래일까지만 표시합니다.</i>",
        ]
    )
