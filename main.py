...
def build_messages(
    ...
    for index, theme in enumerate(themes, 1):
        ...
        leader_lines = [
            (
                f"{_trend_icon(item.change_pct)} <code>{html.escape(item.ticker)}</code>  "
                f"<b>{_signed(item.change_pct)}</b>  | 거래대금 {item.value_ratio:.1f}배"
            )
            for item in leaders
        ]
        kr_text = " · ".join(
            f"{html.escape(item['name'])}({html.escape(item['code'])})"
            for item in theme.kr_stocks
        )
        header.extend(
            [
    ...
    for index, item in enumerate(kr_top10, 1):
        ...
        if theme_name:
            duplicates.append(
                f"{html.escape(item['name'])}({html.escape(theme_name)})"
            )
        domestic.append(
            f"{index:02d}. {new_tag}{html.escape(item['name'])} "
