...
def toss_kr_stock_link(name: str, ticker: str) -> str:
    normalized = str(ticker).strip().upper()
    label = html.escape(f"{name}({normalized})")
    if not re.fullmatch(r"[0-9A-Z]{6}", normalized):
        return label
    url = f"https://www.tossinvest.com/stocks/A{normalized}/order"
...
class Candidate:
    ...
    def amount_rank_compact(self) -> dict[str, Any]:
        return {
            "ticker": self.ticker,
            "name": self.name,
            "rank": self.amount_rank,
            "price": self.price,
...
class IntradayScanner:
    def __init__(self, client: KISClient, resolver: ThemeResolver) -> None:
        ...
        self.amount_top10: dict[str, list[Candidate]] = {}
    ...
    def scan_market(self, market: str, market_code: str) -> list[Candidate]:
        ...
        amount_top10: list[Candidate] = []
        for position, row in enumerate(amount_rows, 1):
            rank = integer(row.get("data_rank"), position)
            if rank > 10:
                continue
            ticker = row_code(row)
            if not ticker:
            ...
            amount_top10.append(
                Candidate(
                    ...
                    name=name,
                    market=market,
                    price=integer(row.get("stck_prpr"), 0),
                    change_pct=number(row.get("prdy_ctrt")),
                    amount_won=number(row.get("acml_tr_pbmn")),
                    amount_rank=rank,
                    theme=self.resolver.resolve(ticker),
                )
            )
        self.amount_top10[market] = sorted(
            amount_top10, key=lambda item: item.amount_rank
        )[:10]
...
def candidate_from_amount_state(market: str, item: dict[str, Any]) -> Candidate:
    return Candidate(
        ticker=str(item.get("ticker", "")),
        name=str(item.get("name", "")),
        market=market,
        price=integer(item.get("price"), 0),
        change_pct=number(item.get("change_pct")),
        amount_won=number(item.get("amount_won")),
        amount_rank=integer(item.get("rank"), 99),
        theme=str(item.get("theme", "미분류")),
    )


def update_amount_rank_state(
    market: str, market_state: dict[str, Any], current: list[Candidate]
) -> list[AmountRankChange]:
    previous = market_state.get("amount_top10", [])
    market_state["amount_top10"] = [item.amount_rank_compact() for item in current]
    if not previous:
        return []

    previous_by_ticker = {
        str(item.get("ticker", "")): item for item in previous if item.get("ticker")
    }
    current_by_ticker = {item.ticker: item for item in current}
    changes: list[AmountRankChange] = []

    for item in current:
        old = previous_by_ticker.get(item.ticker)
        if old is None:
            changes.append(
                AmountRankChange(item=item, previous_rank=None, current_rank=item.amount_rank)
            )
            continue
        old_rank = integer(old.get("rank"), 99)
        if old_rank != item.amount_rank:
            changes.append(
                AmountRankChange(
                    item=item,
                    previous_rank=old_rank,
                    current_rank=item.amount_rank,
                )
            )

    for ticker, old in previous_by_ticker.items():
        if ticker in current_by_ticker:
            continue
        old_item = candidate_from_amount_state(market, old)
        changes.append(
            AmountRankChange(
                item=old_item,
                previous_rank=old_item.amount_rank,
                current_rank=None,
            )
        )
    ...
    return sorted(
        changes,
        key=lambda change: (
            change.current_rank is None,
            change.current_rank if change.current_rank is not None else 99,
            change.previous_rank if change.previous_rank is not None else 99,
        ),
    )
...
def format_candidate(index: int, item: Candidate) -> str:
    ...
    disclosure = (
        f"\n공시: {html.escape(item.disclosure)}" if item.disclosure else ""
    )
    stock_link = toss_kr_stock_link(item.name, item.ticker)
    return (
        f"<b>{index}순위</b> | {stock_link} | {item.price:,}원 "
        f"<b>({item.change_pct:+.2f}%)</b>\n"
        f"테마: {html.escape(item.theme)} | 판단: <b>{html.escape(item.decision)}</b> "
        f"| 점수 {item.score:.1f}\n"
...
def build_alert(
    ...
    lines = [f"🔔 <b>장중 주도 흐름 변화</b> {now:%H:%M:%S}"]
    for market in ("KOSPI", "KOSDAQ"):
        lines.extend(["", f"<b>[{market}]</b>"])
        ...
        lines.append(f"변화: {html.escape(reason)}")
    ...
    for leader in changed_leaders[:2]:
        ...
        if headline:
            headlines.append(
                f"- {html.escape(leader.name)}: {html.escape(headline)}"
            )
    if headlines:
        lines.extend(["", "<b>보조 뉴스</b>", *headlines])
    lines.extend(
        [
            "",
...
def format_amount_won(value: float) -> str:
...
def format_amount_rank_change(change: AmountRankChange) -> str:
    item = change.item
    stock_link = toss_kr_stock_link(item.name, item.ticker)
    if change.previous_rank is None:
        marker = "🆕"
        movement = f"신규진입 → <b>{change.current_rank}위</b>"
    elif change.current_rank is None:
        marker = "⬛"
        movement = f"<b>{change.previous_rank}위</b> → TOP 10 이탈"
    else:
        arrow = "🔺" if change.movement > 0 else "🔻"
        marker = arrow * abs(change.movement)
        direction = "순위상승" if change.movement > 0 else "순위하락"
        movement = (
            f"{direction} <b>{change.previous_rank} → {change.current_rank}</b> "
            f"({abs(change.movement)}계단)"
        )

    details = ""
    if change.current_rank is not None:
        details = (
            f"\n현재가 {item.price:,}원 | <b>{item.change_pct:+.2f}%</b> "
            f"| 거래대금 {format_amount_won(item.amount_won)}"
            f"\n테마: {html.escape(item.theme)}"
        )
    return f"{marker} {stock_link}\n{movement}{details}"


def build_amount_rank_alert(
    changes: dict[str, list[AmountRankChange]], now: datetime | None = None
) -> str:
    timestamp = now or datetime.now(KST)
    lines = [f"📊 <b>실시간 거래대금 TOP 10 변동</b> {timestamp:%H:%M:%S}"]
    for market in ("KOSPI", "KOSDAQ"):
        market_changes = changes.get(market, [])
        if not market_changes:
            continue
        lines.extend(["", f"<b>[{market}]</b>"])
        for change in market_changes:
            lines.extend([format_amount_rank_change(change), ""])
        if lines[-1] == "":
            lines.pop()
    lines.extend(["", "※ 30초 스캔 사이에 확인된 순위 변동을 한 메시지로 묶었습니다."])
    return "\n".join(lines)
...
def run(until: clock_time, state_path: Path, dry_run: bool) -> None:
    ...
    while datetime.now(KST) < end_at:
        ...
        try:
            ...
            amount_rank_changes: dict[str, list[AmountRankChange]] = {}
            for market, candidates in snapshots.items():
                market_state = state["markets"].setdefault(market, {})
                rank_changes = update_amount_rank_state(
                    market, market_state, scanner.amount_top10.get(market, [])
                )
                if rank_changes:
                    amount_rank_changes[market] = rank_changes
                changed, reasons = update_market_state(market_state, candidates, now)
                if changed:
                    changes[market] = reasons
            save_state(state_path, state)

            if amount_rank_changes:
                rank_message = build_amount_rank_alert(amount_rank_changes, now)
                if dry_run:
                    print(rank_message, flush=True)
                else:
                    send_telegram(rank_message)
    ...
    try:
        ...
        if not arguments.dry_run and TELEGRAM_BOT_TOKEN and TELEGRAM_CHAT_ID:
            try:
                send_telegram(
                    html.escape(
                        f"⚠️ 장중 감시기 중단: {type(exc).__name__} — {exc}"
                    )
                )
            except Exception:
